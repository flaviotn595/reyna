 ...
 class YT {
     ...
     /**
      * verifica se é link yt
      * @param {string|URL} url youtube url
      * @returns Retorna true se for o URL do YouTube fornecido.
      */
     static isYTUrl = (url) => {
     ...
     static getVideoID = (url) => {
         if (!this.isYTUrl(url)) throw new Error('não é o URL do YouTube')
         ...
         NodeID3.write(
             {
                 ...
                 image: {
                     ...
                     description: `Capa do ${Metadata.Title}`,
                 },
     ...
     static mp3 = async (url, metadata = {}, autoWriteTags = false) => {
         try {
             ...
             stream.on('progress', (chunkLength, downloaded, total) => {
                 ...
                 process.stdout.write(`${(percent * 100).toFixed(2)}% Baixando `);
                 process.stdout.write(`(${(downloaded / 1024 / 1024).toFixed(2)}MB de ${(total / 1024 / 1024).toFixed(2)}MB)\n`);
                 process.stdout.write(`Baixado nesse momento: ${downloadedMinutes.toFixed(2)}minutos`);
                 process.stdout.write(`, tempo restante estimado: ${estimatedDownloadTime.toFixed(2)}minutos `);
